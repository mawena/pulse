...
- **[2026-07-22]** : ⚡ **Temps réel généralisé à toutes les vues.** Auparavant seuls Dashboard et Services système étaient en WebSocket ; Processus, Services LNMP et Jobs restaient en polling. Ajout des événements `ProcessesUpdated` (canal `processes`, perm `read/process`) et `JobsUpdated` (canal `jobs`, perm `read/system`), diffusés par `pulse:stream` avec cadences dédiées (processus & jobs ~2 s, services ~15 s, métriques chaque tick). Composable frontend réutilisable `lib/useRealtime.js` (abonnement canal privé + filet de sécurité polling 30 s + nettoyage au démontage) appliqué aux 4 vues ; LNMP réutilise le champ `lnmp` de `ServicesUpdated` (aucun flux supplémentaire). Le switch « Auto (10s) » des processus devient « Temps réel ». Vérifié bout-en-bout avec Reverb réel (processus 616, jobs, services LNMP+systemd reçus sur leurs canaux). 29/29 tests Pest.

- **[2026-07-22]** : ⚡ **Temps réel WebSocket + nouvelles vues de monitoring.** Fix critique : menu latéral vide (le getter `can()` ne gérait pas le format tableau des ability_rules CASL). WebSocket : Laravel Reverb installé, événements `MetricsUpdated` (3 s) / `ServicesUpdated` (15 s) diffusés par la commande `pulse:stream` (boucle backend, `--once` pour les tests), canaux privés `metrics`/`services` autorisés par permissions CASL via `/api/broadcasting/auth` (Sanctum Bearer), limites de payload Reverb à 256 Ko. Frontend : `lib/echo.js` (authorizer axios), store metrics en WS avec **fallback polling automatique** (badge LIVE vert / POLL orange dans la topbar). Nouvelles vues : **Services système** (toutes les unités systemd, filtres, temps réel), **Jobs** (en attente/en cours/échoués + retry audité), **Rôles & Permissions** (CRUD complet avec cases à cocher de permissions). Navigation groupée Monitoring/Administration + menu « Plus » sur la bottom-nav mobile. Endpoints : `GET /api/system-services`, `GET /api/jobs`, `POST /api/jobs/{uuid}/retry`. Déploiement : unités systemd `pulse-reverb`/`pulse-stream`, proxy WebSocket `/app/` dans le vhost nginx, doc mise à jour. `composer dev` lance désormais reverb + stream. Testé bout-en-bout (Reverb réel + broadcast). **28/28 tests Pest.**
